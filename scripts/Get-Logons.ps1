...
Get-WinEvent -FilterHashTable @{LogName='Security'; id=4624} | Where-Object {(($_.Properties[8].Value -eq 2) -or ($_.Properties[8].Value -eq 10)) -and ($_.Properties[18].Value -ne "-")} | Format-Table $tableFormat -AutoSize -Wrap | Out-File $reportPath
